...
impl KrakenAdapter {
    /// `config/instruments.yaml` carries the WS v2 spellings, which is what this
    /// adapter speaks, so every `native` goes on the wire byte for byte. There
    /// was a `XBT/` -> `BTC/`, `XDG/` -> `DOGE/` translation here while the
    /// registry had to keep the v1 spellings for the Kotlin v1 handlers; those
    /// retired (ADR-019) and it went with them.
...
mod tests {
    ...
    ///
    /// `config/instruments.yaml` carried Kraken's WS **v1** spellings until the
    /// Kotlin v1 handlers retired, and this adapter rewrote `XBT/` -> `BTC/` and
    /// `XDG/` -> `DOGE/` on construction. The registry now carries the v2
    /// spellings, the translation is gone, and `native` goes out byte for byte -
    /// which is the contract `config/instruments.yaml` states and the only way a
    /// registry typo shows up as a venue error rather than as silent aliasing.
    #[test]
    fn registry_natives_reach_the_wire_untranslated() {
        let yaml = "version: 2\ninstruments:\n  kraken:\n                        - { native: BTC/USD, canonical: BTC/USD }\n                        - { native: DOGE/USD, canonical: DOGE/USD }\n                        - { native: ETH/USD, canonical: ETH/USD }\n";
        let mut a = KrakenAdapter::new(Instruments::parse(yaml, Exchange::Kraken).unwrap());
        a.begin_connection("test-conn");

        assert_eq!(a.symbols(), vec!["BTC/USD", "DOGE/USD", "ETH/USD"]);
        ...
        let OutRecord::Trade(t) = &h.records[1] else {
            panic!("no trade record for BTC/USD")
        };
        assert_eq!(t.symbol, "BTC/USD", "wire spelling, per the .avsc contract");
        assert_eq!(t.canonical_symbol, "BTC/USD");
    }

    /// A v1 spelling in the registry is a registry bug, and it must present as
    /// one: subscribed verbatim (so Kraken answers "Currency pair not supported
    /// XBT/USD"), and unmatched by any v2 frame that arrives. The failure mode
    /// this replaces was worse - the adapter quietly rewrote it and the mistake
    /// never surfaced.
    #[test]
    fn a_v1_spelling_is_not_aliased_and_matches_nothing() {
        let yaml = "version: 2\ninstruments:\n  kraken:\n                        - { native: XBT/USD, canonical: BTC/USD }\n";
        let mut a = KrakenAdapter::new(Instruments::parse(yaml, Exchange::Kraken).unwrap());
        a.begin_connection("test-conn");

        assert_eq!(a.symbols(), vec!["XBT/USD"], "no rewrite on the way out");
        let book_subscribe = &a.subscribe_messages()[1];
        assert!(
            book_subscribe.contains("\"XBT/USD\"") && !book_subscribe.contains("BTC/USD"),
            "the registry byte string is what goes on the wire: {book_subscribe}"
        );

        // ... and the v2 frame the venue would send for the pair it *does* know
        // resolves to nothing, because we never subscribed to that symbol.
        let frame = r#"{"channel":"trade","type":"update","data":[{"symbol":"BTC/USD","side":"buy","price":78568.7,"qty":0.001,"ord_type":"limit","trade_id":1,"timestamp":"2026-08-26T11:34:06.075373Z"}]}"#;
        let h = a.handle_frame(frame.as_bytes(), 1);
        assert!(
            !h.records.iter().any(|r| matches!(r, OutRecord::Trade(_))),
            "an unsubscribed symbol must not produce a trade"
        );
    }
