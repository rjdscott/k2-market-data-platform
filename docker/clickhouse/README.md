...
| `ddl/10-gold-tables.sql` | **The contract.** `gold.trades`, `gold.book_top20`, `gold.feed_errors`, the on-read views `gold.ohlcv_live(bucket)` and `gold.bbo_live`. The only DDL CI applies |
