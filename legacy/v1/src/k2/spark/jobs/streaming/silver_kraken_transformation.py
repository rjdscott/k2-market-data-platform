#!/usr/bin/env python3
"""Silver Kraken Transformation Job - Bronze → Silver with DLQ.
...
4. Routes valid records to silver_kraken_trades
...
Usage:
    docker exec k2-spark-master /opt/spark/bin/spark-submit \
      ...
      --executor-cores 2 \
      --num-executors 2 \
      /opt/k2/src/k2/spark/jobs/streaming/silver_kraken_transformation.py
...
deserialize_kraken_raw_to_v2 = avro_module.deserialize_kraken_raw_to_v2
...
def main():
    """Main entry point."""
    print("\n" + "=" * 70)
    print("K2 Silver Transformation - Kraken")
    print("Bronze → Silver (Validated) + DLQ (Invalid)")
    print("=" * 70)
    print("\nConfiguration:")
    print("  • Source: bronze_kraken_trades (raw bytes)")
    print("  • Target: silver_kraken_trades (validated V2 schema)")
    ...
    spark = create_spark_session("K2-Silver-Kraken-Transformation")
    ...
    try:
        ...
        # Deserialize Kraken raw Avro and transform to V2
        # UDF handles: strip 5-byte header, deserialize Kraken raw, transform to V2
        print("✓ Deserializing Kraken raw and transforming to V2...")
        deserialized_df = (
            bronze_df.withColumn("trade", deserialize_kraken_raw_to_v2(col("raw_bytes")))
            .select(
        ...
        _dlq_query = write_to_dlq(
            ...
            checkpoint_location="/checkpoints/silver-kraken-dlq/",
        )

        print("\n" + "=" * 70)
        print("Silver Kraken Transformation - RUNNING")
        print("=" * 70)
        print("\nMonitoring:")
