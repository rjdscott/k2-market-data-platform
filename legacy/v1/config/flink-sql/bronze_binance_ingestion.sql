-- ==============================================================================
-- Flink SQL Job: Binance Bronze Ingestion
-- ==============================================================================
-- Purpose: Stream raw Binance trades from Kafka to Iceberg Bronze table
-- Source: Kafka topic 'binance.raw' (Avro with Schema Registry)
-- Sink: Iceberg table 'market_data.bronze_binance_trades_flink'
...
SET 'pipeline.name' = 'bronze_binance_ingestion';
...
CREATE TABLE kafka_source_binance (
    ...
    'properties.group.id' = 'flink-bronze-binance',
...
--       -f /opt/flink/sql/bronze_binance_ingestion.sql
--
-- The INSERT INTO statement creates a long-running Flink job that:
-- 1. Consumes from 'binance.raw' Kafka topic
...
-- - Job name: "bronze_binance_ingestion"
...
-- 3. Exact Kafka topic name: market.crypto.trades.binance.raw
