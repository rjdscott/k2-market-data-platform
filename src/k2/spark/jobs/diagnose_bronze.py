...
bronze_df.select("message_key", "topic", "partition", "offset", "kafka_timestamp").show(
    5, truncate=False
)

print("\nChecking Avro payload sizes:")
bronze_df.select(
    col("message_key"), col("avro_payload").cast("string").substr(1, 50).alias("payload_preview")
...
bronze_df.select(
    "message_key", col("avro_payload").cast("binary").alias("payload")
