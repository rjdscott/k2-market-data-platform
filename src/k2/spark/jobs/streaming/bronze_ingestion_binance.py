#!/usr/bin/env python3
"""Bronze Ingestion Job - Binance (Kafka → Bronze Iceberg).

This job streams raw Binance trade data from Kafka and writes to Bronze Iceberg table.
No transformations - pure raw data storage for audit trail and reprocessing.

Architecture:
- Source: Kafka topic market.crypto.trades.binance.raw
- Target: bronze_binance_trades (Iceberg table)
- Checkpoints: /checkpoints/bronze-binance/
...
def main():
    ...
    print("=" * 70)
    print("\nConfiguration:")
    print("  • Source: Kafka topic market.crypto.trades.binance.raw")
    ...
    try:
        # Fetch Binance raw schema from Schema Registry
        # Note: The schema subject should match what RawBinanceProducer registered
        avro_schema = fetch_avro_schema_from_registry("market.crypto.trades.binance.raw-value")
        ...
        print("Starting Bronze writer for Binance...")
        bronze_table = "iceberg.market_data.bronze_binance_trades"
