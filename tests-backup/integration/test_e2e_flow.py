...
Usage:
    pytest tests-backup/integration/test_e2e_flow.py -v -s -m integration
"""
...
# Test configuration
TEST_COMPANY_ID = "7181"  # DVN - low volume, fast for tests-backup
TEST_SYMBOL = "DVN"
...
class TestSampleDataAvailability:
    """Verify sample data is accessible for E2E tests-backup."""
...
class TestE2EDataFlow:
    """
    End-to-end tests-backup for complete data pipeline.

    Note: These tests-backup require Docker services running:
...
1. Quick tests-backup (no Docker required):
   pytest tests-backup/integration/test_e2e_flow.py -v -k "TestSampleData or TestDataTransform"

2. Full E2E tests-backup (requires Docker):
   make docker-up
   make init-infra
   pytest tests-backup/integration/test_e2e_flow.py -v -m integration

3. Load data and test full pipeline:
   make docker-up
   make init-infra
   python scripts/demo.py --quick
   pytest tests-backup/integration/test_e2e_flow.py -v
...
Expected results:
- All data transformation tests-backup pass
