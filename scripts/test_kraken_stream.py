#!/usr/bin/env python3
"""Test script for Kraken WebSocket streaming.

This script connects to Kraken WebSocket API, streams live trades for
BTC/USD and ETH/USD, and prints the converted v2 trade data.
...
def print_trade(trade: dict) -> None:
    ...
    if trade_count == 1:
        console.print("\n[bold green]✓ Connected to Kraken WebSocket[/bold green]\n")
        console.print(
    ...
    table.add_row("Pair", trade["vendor_data"]["pair"])  # Original Kraken pair
    ...
    table.add_row("Order Type", trade["vendor_data"]["order_type"])  # l=limit, m=market
...
async def main() -> None:
    """Main function to test Kraken WebSocket client."""
    # Parse command line arguments
    import argparse

    parser = argparse.ArgumentParser(description="Test Kraken WebSocket streaming")
    parser.add_argument(
        "--symbols",
        nargs="+",
        default=["BTC/USD", "ETH/USD"],
        help="Symbols to stream (default: BTC/USD ETH/USD)",
    )
    args = parser.parse_args()

    console.print("[bold blue]Testing Kraken WebSocket Client[/bold blue]\n")
