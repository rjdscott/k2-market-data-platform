#!/bin/bash
# Start Silver Kraken Transformation Job v2 (Bronze → Silver with Schema Registry + Professional Partitioning)
#
# Features:
...
docker exec k2-spark-master bash -c "cd /opt/k2 && AWS_REGION=us-east-1 /opt/spark/bin/spark-submit \
  ...
  src/k2/spark/jobs/streaming/silver_transformation_v2.py kraken"
