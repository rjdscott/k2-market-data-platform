#!/usr/bin/env python3
"""Kraken streaming service - production daemon.
...
Architecture:
    Kraken WebSocket → on_message → MarketDataProducer → Kafka

Features:
- Real-time trade streaming from Kraken
- Automatic reconnection with exponential backoff
- V2 schema conversion with vendor_data
- XBT → BTC normalization for cross-exchange compatibility
- Idempotent Kafka production (at-least-once with deduplication)
- Graceful shutdown (SIGINT/SIGTERM handlers)
- Comprehensive logging and statistics

Usage:
    # Stream default symbols (from config or BTC/USD, ETH/USD)
    python scripts/kraken_stream.py

    # Stream custom symbols
    python scripts/kraken_stream.py --symbols BTC/USD ETH/USD DOT/EUR

    # Run as daemon with info logging
    python scripts/kraken_stream.py --daemon --log-level info
...
Environment Variables:
    ...
    K2_KRAKEN_WEBSOCKET_URL: Kraken WebSocket URL (default: wss://ws.kraken.com)
    K2_KRAKEN_SYMBOLS: Comma-separated symbols (default: BTC/USD,ETH/USD)
"""
...
async def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Kraken WebSocket streaming service",
...
Environment Variables:
  ...
  K2_KRAKEN_SYMBOLS             Comma-separated symbols
    ...
    if not args.daemon:
        console.print("\n[bold blue]Kraken Streaming Service[/bold blue]")
        console.print("=" * 50)
    ...
    def handle_trade(trade: dict) -> None:
        ...
        Args:
            trade: v2 Trade record from convert_kraken_trade_to_v2()
        """
        nonlocal trade_count, error_count

        try:
            # Produce to Kafka topic: market.crypto.trades.kraken
            ...
            if trade_count % 50 == 0:
                ...
                if not args.daemon:
                    console.print(
                        ...
                        f"(errors: {error_count}) | Last: {trade['symbol']} "
                        f"({trade['vendor_data']['pair']}) @ {trade['price']}"
                    )
            ...
            logger.error(
                ...
                pair=trade.get("vendor_data", {}).get("pair"),
    ...
    client = KrakenWebSocketClient(
        ...
        failover_urls=config.kraken.failover_urls,
        reconnect_delay=config.kraken.reconnect_delay,
        max_reconnect_attempts=config.kraken.max_reconnect_attempts,
        health_check_interval=config.kraken.health_check_interval,
        health_check_timeout=config.kraken.health_check_timeout,
        enable_circuit_breaker=True,  # Enable circuit breaker for production resilience
    )

    logger.info(
        "kraken_client_created",
        symbols=symbols,
        websocket_url=config.kraken.websocket_url,
        failover_urls=config.kraken.failover_urls,
        reconnect_delay=config.kraken.reconnect_delay,
        max_reconnect_attempts=config.kraken.max_reconnect_attempts,
        health_check_interval=config.kraken.health_check_interval,
