...
- Chaos: `make chaos` exits 0 — every script sees its expected alert in `curl -s localhost:9090/api/v1/alerts | jq -r '.data.alerts[].labels.alertname'` and the stack returns to green within the script's stated bound; `docs/architecture/16-failure-modes.md` has no empty `detection signal`, `recovery step` or `proof` cell (`awk -F'|' '/^\|/ && NF>5 {for(i=2;i<NF;i++) if($i ~ /^ *$/) exit 1}' docs/architecture/16-failure-modes.md`).
- Isolation: noisy-neighbour p99 during compaction and the quiet baseline are both in `15-capacity-model.md`, each with its `curl`/`clickhouse-client` command; the compaction run is pinned off the capture cpuset (`docker inspect -f '{{.HostConfig.CpusetCpus}}' k2-spark-iceberg k2-capture-binance` shows disjoint sets).
...
- **The exit criteria themselves are unexecuted.** `raw`, `bronze` and `audit` exist
  ...
  `docs/architecture/16-failure-modes.md` says "written; not yet run", every threshold in
...
- **The noisy-neighbour experiment (Scope bullet 8, Verification bullet 3) was deferred
  ...
  commands are the dated note at the foot of `15-capacity-model.md`. Verdict there: p50 flat,
...
_Phase D landed 2026-08-27 — commit e2a2651, tag v3-phase-d. Follow-ups measured the same night: first nightly maintenance exit 0 after 249 s at 03:04Z with 0 audit failures; the four lake chaos scripts green (`scripts/chaos/results/2026-08-27.tsv`, proof cells in 16-failure-modes.md)._
