...
- Topics (`redpanda-init`): `market.crypto.raw.{ex}` (RawMessage, 48h + bytes cap), `market.crypto.trades.{ex}` (Trade, 7d), `market.crypto.book.{ex}` (BookSnapshotL2, 7d); 12 partitions each; key = canonical symbol; delete `.raw` JSON topics only after CH cutover. *(Landed as `market.crypto.v3.{raw,trades,book}.<ex>` — v2 owns `market.crypto.trades.<ex>`; see ADR-018 Appendix A, Deviations.)*
