...
| Lakekeeper's compose healthcheck is the in-image `lakekeeper healthcheck` subcommand (`CMD`, not `CMD-SHELL`) | S9 reported no healthcheck subcommand; wiring it into compose showed `quay.io/lakekeeper/catalog:v0.13.3` does list `healthcheck` alongside `migrate`/`serve`. There is still no shell, so `test:` must be exec-form. `GET /health` (200) stays the external probe. |
| v3 topics are `market.crypto.v3.{raw,trades,book}.<ex>`, not `market.crypto.{raw,trades,book}.<ex>` | `market.crypto.trades.<ex>` is the live v2 normalized topic (`docker-compose.yml` redpanda-init, registry subject `market.crypto.trades.<ex>-value` = NormalizedTrade v1). Registering `trade.avsc` there returns `{"is_compatible":false}` under BACKWARD_TRANSITIVE, and `alter-config` on the existing topic silently rewrote v2 retention in a dry run. The parallel-run window in this ADR needs both producers on separate topics anyway; the prefix is uniform across all nine (`V3_PREFIX` in `docker/redpanda/init.sh`). |
| Lake namespaces `raw`/`bronze`/`audit` are created by `lake-init` over the REST catalog API; no `lake-ddl` Spark one-shot in Phase B | Three HTTP calls against a service already running versus a ~20 s JVM start for zero tables. Table DDL arrives with Phase D and can pay for the JVM then. Namespace create is `POST /catalog/v1/{warehouse-uuid}/namespaces` — the warehouse *name* returns `400 WarehouseIdIsNotUUID`; the UUID is `defaults.prefix` from `GET /catalog/v1/config?warehouse=k2`. |
| `schemas/avro/normalized-trade.avsc` is kept | `KafkaProducerService.kt:115` parses it at producer start in all three v2 handlers; deleting it in Phase B would fail the "v2 still green" exit. It goes with the Kotlin handlers in Phase C. |
