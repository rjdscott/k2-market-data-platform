# Redpanda and the wire contracts
...
| Retention sized from measurement | `init.sh` arithmetic cites the per-venue GB/day; `capacity-model.md` scores it |
...
- **Retention as deadline.** An ingest outage past 48 h loses raw frames; the lake records
  the hole rather than pretending ([lake-ingest.md](lake-ingest.md)).
