...
|---|---|---|---|
| CPU limits | 35–40 cores | **15.35** | −56 to −62% |
| RAM limits | 45–50 GB | **22.125 GB** | −51 to −56% |
| Long-lived services | 18–20 | **15** (+4 one-shot) | −17 to −25% |
...
Fits the mandate on both axes, with 0.65 CPU and 17.875 GB of headroom.
...
| 15.5 CPU / 19.5 GB across 11 services | 15.35 CPU / 22.125 GB across 15 services (+4 one-shot) | ~ CPU came in under; RAM over by 2.625 GB, mostly Prometheus and the third exchange |
...
3. **Constraints beat intentions.** "Reduce resource usage" produces tuning. "16 cores, 40 GB, one host" produces architecture. The budget in [ADR-010](adr/ADR-010-resource-budget.md) was checked at every phase boundary, which is why the answer at the end was 15.35 and not 22.
