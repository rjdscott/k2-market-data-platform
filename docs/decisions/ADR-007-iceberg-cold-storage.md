...
Iceberg — Hadoop catalog on a bind-mounted local warehouse (MinIO provisioned, unused by the offload) — was retained as the cold tier and holds all 10 tables (3 bronze, 1 silver, 6 gold). Two deviations:

- **Hadoop catalog, not REST catalog + PostgreSQL.** The `iceberg-rest` container and its Postgres catalog store were dropped in favour of a file-based Hadoop catalog rooted at the bind-mounted local warehouse ([ADR-013](ADR-013-pragmatic-iceberg-version-strategy.md)). For a single-writer batch offload this removes a service and a failure mode at no functional cost. PostgreSQL is still in the stack — as the Prefect metadata store and offload watermark table, not as the Iceberg catalog.
