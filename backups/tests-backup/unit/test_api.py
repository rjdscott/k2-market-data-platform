"""Unit tests-backup for K2 REST API.

Tests the API endpoints, middleware, and response models using FastAPI's
TestClient. These tests-backup mock the QueryEngine and ReplayEngine to avoid
...
class TestPostTradesQuery:
    ...
    def test_post_trades_query_field_selection(self, client, auth_header):
        """POST /v1/trades/query should support field selection (v2 fields)."""
        response = client.post(
            "/v1/trades/query",
            headers=auth_header,
            json={
...
class TestPostSnapshotQuery:
    ...
    def test_post_snapshot_query_basic(self, client, auth_header, mock_replay_engine):
        ...
        mock_replay_engine.query_at_snapshot.return_value = [
            {
                "message_id": "snapshot-msg-001",
                "trade_id": "snapshot-trade-001",
                "symbol": "BTCUSDT",
                "exchange": "BINANCE",
                "asset_class": "crypto",
                ...
                "trade_conditions": None,
                "source_sequence": 12345,
                "ingestion_timestamp": datetime(2024, 1, 15, 10, 0, 1),
                "platform_sequence": None,
                "vendor_data": None,
            },
        ]
